...
class AuthRepo {
    ...
    suspend fun shareMailForPasswordReset(email: String, context: IGlobalCallbacks) {
        emailAuthManager?.sendThePasswordResetMail(email, context)
    }

    suspend fun doPhoneVerification(
        otp: String,
        otpToken: String,
    ...
    suspend fun setUpPhoneAuthentication(
        phoneNumber: String,
        activity: Activity,
        listener: IPhoneAuthenticatorCallbacks
    ) {
        phoneAuthManager?.setUpPhoneAuthenticator(phoneNumber, activity, listener)
    }

    suspend fun setUpResendTokenAuthentication(
        phoneNumber: String,
        activity: Activity,
