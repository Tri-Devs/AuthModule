...
class LoginFragment : Fragment(), AuthenticationCallback, IGlobalCallbacks {
    ...
    override fun onCreateView(
        ...
        if(savedInstanceState != null){
            binding.etEmail.setText( savedInstanceState.getString(Constants.USER_EMAIL).toString())
            binding.etPassword.setText( savedInstanceState.getString(Constants.USER_PASSWORD).toString())
            binding.etPassword.setText( savedInstanceState.getString(Constants.USER_PASSWORD).toString())
    ...
    private fun bottomSheetForEmailSetup() {
        bBinding?.apply {
            ...
            btnDone.setOnClickListener {
                ...
                it.closeKeyboard()
                        loginViewModel.sendMailForPasswordReset(
                            bBinding?.forgotEmail?.text.toString(),
                            this@LoginFragment
                        )
    ...
    * */
    ...
    override fun successCallback() {
        startOnMainThread {
            "Mail shared successfully on ${bBinding?.forgotEmail?.text.toString()}".debugToast(
                requireContext(),
                true
            )
            bottomSheet?.dismiss()
    ...
    override fun failureCallback() {
        startOnMainThread {
            "Please try again after some time".debugToast(requireContext(), true)
            bottomSheet?.dismiss()
        }
    }
